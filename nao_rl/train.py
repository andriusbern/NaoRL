...
if __name__ == "__main__":
    ...
    parser.add_argument('render_mode', type=int, help='Rendering mode: [0] - no rendering, [1] - render first worker, [2] - render all workers', choices=[0,1,2])
    parser.add_argument('-p', '--live_plot', action='store_true', help='Live plotting of rewards during training')
    ...
    try:
        ...
        print "No default parameters in 'settings.py' for {} environment.".format(env_name)
        parameters = {}
    ...
    if algorithm == 'ppo':
        model = PPO(env_name, render_mode, live_plot, **parameters)
    if algorithm == 'a3c':
        model = A3C(env_name, render_mode, live_plot, **parameters)
    ...
    log.create_from_model(model, parameters)
    log.save()
    model.plot_rewards()
